...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
  ...
  const deleteUrl = async (_id: string) => {
    const response = await axios.delete(`${serverUrl}/shortUrl/${_id}`);
  ...
  const renderTableData = () => {
    return data.map((item) => {
      return (
        <tr
          key={item._id}
          className="border-b text-gray-800 bg-slate-300 hover:bg-white hover:text-black"
        >
          ...
          <td className="px-6 py-3">
            <div className="flex content-center">
              <div
                ...
                onClick={() => copyClipboard(item.shortUrl)}
              >
                <svg
